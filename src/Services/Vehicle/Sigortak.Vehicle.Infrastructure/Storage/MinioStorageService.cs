...
namespace Sigortak.Vehicle.Infrastructure.Storage;

public class MinioStorageService : IPolicyStorageService
{
    ...
    public MinioStorageService(IConfiguration configuration, ILogger<MinioStorageService> logger)
    {
        ...
        _bucketName = configuration["Minio:BucketName"] ?? "policy-documents";
    ...
    public async Task<string> UploadPolicyDocumentAsync(Stream fileStream, string fileName, string contentType, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            // Build access URL
            return $"/api/v1/policies/document/{fileName}";
        }
        catch (Exception ex)
        {
    ...
    public async Task<(Stream FileStream, string ContentType)> DownloadPolicyDocumentAsync(string fileName, CancellationToken cancellationToken = default)
    {
        try
        {
